...
function openAura.plugin:IncludeEntities(directory)
	for k, v in pairs( _file.FindInLua(directory.."/entities/entities/*") ) do
		if (v != ".." and v != ".") then
			...
			elseif ( _file.Exists("lua_temp/"..directory.."/entities/entities/"..v.."/cl_auto.lua", true)
			or _file.Exists("gamemodes/"..directory.."/entities/entities/"..v.."/cl_auto.lua", true) ) then
...
function openAura.plugin:IncludeEffects(directory)
	for k, v in pairs( _file.FindInLua(directory.."/entities/effects/*") ) do
		if (v != ".." and v != ".") then
			if (SERVER) then
				if ( _file.Exists("gamemodes/"..directory.."/entities/effects/"..v.."/cl_auto.lua", true) ) then
					AddCSLuaFile(directory.."/entities/effects/"..v.."/cl_auto.lua");
				end;
			elseif ( _file.Exists("lua_temp/"..directory.."/entities/effects/"..v.."/cl_auto.lua", true)
			or _file.Exists("gamemodes/"..directory.."/entities/effects/"..v.."/cl_auto.lua", true) ) then
				EFFECT = {};
...
function openAura.plugin:IncludeWeapons(directory)
	for k, v in pairs( _file.FindInLua(directory.."/entities/weapons/*") ) do
		if (v != ".." and v != ".") then
			...
			if (SERVER) then
				if ( _file.Exists("gamemodes/"..directory.."/entities/weapons/"..v.."/sh_auto.lua", true) ) then
					include(directory.."/entities/weapons/"..v.."/sh_auto.lua");
				end;
			elseif ( _file.Exists("lua_temp/"..directory.."/entities/weapons/"..v.."/sh_auto.lua", true)
			or _file.Exists("gamemodes/"..directory.."/entities/weapons/"..v.."/sh_auto.lua", true) ) then
				include(directory.."/entities/weapons/"..v.."/sh_auto.lua");
			end;
...
function openAura.plugin:IncludeDerma(directory)
	for k, v in pairs( _file.FindInLua(directory.."/derma/*") ) do
		if (v != ".." and v != ".") then
			if (SERVER) then
				if ( _file.Exists("gamemodes/"..directory.."/derma/"..v, true) ) then
					AddCSLuaFile(directory.."/derma/"..v);
				end;
			elseif ( _file.Exists("lua_temp/"..directory.."/derma/"..v, true)
			or _file.Exists("gamemodes/"..directory.."/derma/"..v, true) ) then
...
function openAura.plugin:IncludePlugins(directory)
	...
	for k, v in pairs( _file.FindInLua(directory.."/plugins/*") ) do
		if (v != ".." and v != ".") then
			if (CLIENT) then
				if ( _file.IsDir("lua_temp/"..directory.."/plugins/"..v, true) ) then
					self:Include(directory.."/plugins/"..v);
				end;
			elseif ( _file.IsDir("gamemodes/"..directory.."/plugins/"..v, true) ) then
				self:Include(directory.."/plugins/"..v);
			end;
		end;
	end;
...
function openAura.plugin:Include(directory, schema)
	...
	if (SERVER) then
		...
		if ( _file.Exists("gamemodes/"..directory.."/sh_info.lua", true) ) then
		...
		if (!isUnloaded and !isDisabled) then
			if ( _file.Exists("gamemodes/"..directory.."/sv_auto.lua", true) ) then
				include(directory.."/sv_auto.lua");
			end;
		end;
		
		if ( _file.Exists("gamemodes/"..directory.."/cl_auto.lua", true) ) then
		...
		if ( _file.Exists("lua_temp/"..directory.."/sh_info.lua", true) ) then
			include(directory.."/sh_info.lua");
		else
			ErrorNoHalt("OpenAura -> the "..PLUGIN_FOLDERNAME.." plugin has no sh_info.lua.");
		end;
		...
		if (!isUnloaded and !isDisabled) then
			if ( _file.Exists("lua_temp/"..directory.."/cl_auto.lua", true) ) then
